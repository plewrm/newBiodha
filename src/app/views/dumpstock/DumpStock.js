...
const AppTable = () => {
    ...
    const getorderListing = async () => {
                var query = ""
                // const response = await getDataFromApi('/dashboard/dumpStock/dumpStockListing/'+BMId, query);
                const response = await getDataFromApi('/dashboard/dumpStock/dumpStockListing/'+userdetails.user.id, query);
    ...
    return (
        <Container>
            ...
            <Button className="rightalign_btn" variant="outlined"
                color="primary" onClick={() => navigate('/dump-stock/add')}>
               Add New
            </Button>
	                ...
	                <tbody>
	                    {orderListing.map((order, index) => (
	                        <tr key={index}>
	                            ...
	                            <td align="left">
                                    {order.approvalStatus==1 ? 'Approved' : (order.approvalStatus==2 ? 'Rejected':'Pending')}
                                </td>
	                            <td>
                                    <Tooltip title="View">
                                        <IconButton onClick={() => navigate('/dump-stock/0/'+order.id)}>
