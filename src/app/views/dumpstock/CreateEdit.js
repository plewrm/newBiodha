...
const AppTable = () => {
    ...
    const [dumpStockData, setDumpStockData] = useState({})
    ...
    const getskuproperties = async (dataid = '') => {
        var id = dataid ? dataid : formdata.SKU
        setrows([])
        console.log('idres', formdata.SKU)
    ...
    const handleClickAdd = async (id) => {
        ...
        if (response.data.code) {
            ...
            //   console.log('sku Show',response.batchID)
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        var newformdata = {
            ...
            approvalStatus: 0,
            dump_quantity: formdata.dump_qty,
        ...
        response = await postDataFromApi(
            '/dashboard/dumpStock/dumpClosingCreateRequest',
            newformdata
        )
        if (response.data.code) {
            setalermessage(response.data.message)
    ...
    const handledeleteConfirm = async () => {
        var newformdata = {
            ...
            skuId: dumpStockData.skuId,
            dump_quantity:dumpStockData.dump_quantity,
            batchId:dumpStockData.batchId,
        }
        var response = ''

        response = await putDataFromApi(
            'dashboard/dumpStock/approveRejectDumpStock/' + paramid,
            newformdata
        )
        ...
        if (response.data.code) {
            setstatuschange(false)
            if (check_status) {
                setalermessage('Dump Stock Approved Successfully')
            } else {
                setalermessage('Dump Stock Rejected Successfully')
            }
            setalert(true)
            setalerttype('success')
            setredirectorder(true)
    ...
    const getEditDetails = async () => {
        var query = ''
        const response = await postDataFromApi(
            'dashboard/dumpStock/getDumpStockById/' + paramid,
            query
        )
        if (response && response.data.code && response.data.data != null) {
            const viewdata = response.data.data[0]
            viewdata.totalDumpWeight=response.data.totalDumpWeight
            setDumpStockData(viewdata)
        ...
        return (
            <Container>
                <div className="breadcrumb leftalign_breadcrumb">
                    <Breadcrumb routeSegments={[{ name: 'Dump Stock' }]} />
                </div>
                <Button
                    className="rightalign_btn"
                    variant="outlined"
                    color="primary"
                    onClick={() => navigate('/dump-stock')}
                >
                    Back to Dump Stock
                </Button>
                <AlertMessage
                    ...
                    <Grid container spacing={3}>
                        <Grid item lg={6} md={6} sm={12} xs={12} sx={{ mt: 2 }}>
                            <AutoComplete
                                ...
                                defaultValue={getSelectedItem(formdata.SKU,SkuTypeList)}
                                fullWidth
                                options={SkuTypeList}
                            ...
                            <AutoComplete
                               className={paramid ? "view-disabled" : ""}
                                fullWidth
                                options={ZoneList}
                                defaultValue={getSelectedItem(formdata.zoneID,ZoneList)}
                                getOptionLabel={(option) => option.label}
                                onChange={(event, value) =>
                            ...
                            <AutoComplete
                               className={paramid ? "view-disabled" : ""}
                                fullWidth
                                options={BatchList}
                                defaultValue={getSelectedItem(formdata.batchID,BatchList)}
                                getOptionLabel={(option) => option.label}
                                onChange={(event, value) =>
                        ...
                        <Grid item lg={6} md={6} sm={12} xs={12} sx={{ mt: 2 }}>
                            <AutoComplete
                               className={paramid ? "view-disabled" : ""}
                                fullWidth
                                defaultValue={getSelectedItem(formdata.BMId,BMoptions)}
                                options={BMoptions}
                                getOptionLabel={(option) => option.label}
                                onChange={(event, value) =>
                        ...
                        >
                            ...
                            >
                                ...
                                {paramid ? (
                                    <tr>
                                        { dumpStockData.propertydata ? dumpStockData.propertydata.map(
                                            (skuproperties, index) => (
                                ...
                                {paramid ? (
                                        <tr>
                                            ...
                                            <td>{dumpStockData.totalDumpWeight}</td>
                                            {' '}
                                        </tr>
                                ):(
                                    rows.map((rows, index) => (
