...
const AppTable = () => {
    ...
    const [interGodownList, setinterGodownList] = useState([])
    ...
    const getsku_masters = async () => {
        ...
        const response = await getDataFromApi('invoice/intergodownTransfer/getIntergodownTransferList')
        if (response && response.data.code && response.data.data != null) {
            setinterGodownList(response.data.data)
    ...
    const handleSubmit = async (e) => {
        ...
        if (is_edit) {
            response = await putDataFromApi(
                'invoice/intergodownTransfer/updateIntergodownTransferForm/' + edit_id,
                formdata
            )
        } else {
    ...
    return (
        <Container>
            <div className="breadcrumb leftalign_breadcrumb">
                <Breadcrumb routeSegments={[{ name: 'IntergodownTransfer' }]} />
            </div>
            <Button
                className="rightalign_btn"
                variant="outlined"
                color="primary"
                onClick={() => navigate('/IntergodownTransferadd')}
            >
                Add New IntergodownTransfer
            </Button>
            <AlertMessage
                alert={alert}
            ...
            <Box width="100%" overflow="auto">
                ...
                >
                    ...
                    <tbody>
                        {interGodownList.map((order, index) => (
                            <tr key={index}>
                                <td align="left">{index + 1}</td>
                                <td align="left">{order.fromYards}</td>
                                <td align="left">{order.toYards}</td>
                                {/* <td align="left">{order.nameOfYard}</td> */}
                                <td align="left">{order.productGroupName}</td>
                                <td align="left">
                                ...
                                <Tooltip title="View">
                                    ...
                                    <Tooltip title="Edit">
    	                                <IconButton onClick={() => navigate('/IntergodownTransferadd/edit/'+order.id)} >
                                    ...
                                    '' : <Tooltip title="Status"><IconButton onClick={() => navigate('/order/changestatus/'+order.id)}>
            ...
            >
                <ValidatorForm onSubmit={handleSubmit} onError={() => null}>
                    <DialogTitle id="form-dialog-title">
                        {is_edit ? 'Update' : 'Add New'} fromYard
                    </DialogTitle>
                    <DialogContent>
