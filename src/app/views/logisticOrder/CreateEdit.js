...
const AppTable = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        orderDetails.map((list, i) => {
            i = i + 1
            var data = {
                skuMasterId: formdata.skuids['skuids_' + i],
                stock_check: formdata.stockcheck['stockcheck_' + i],
                qtyPcs: formdata.quantity_pcsarr['quantity_pcs_' + i],
                qtyMT: formdata.quantity_mtarr['quantity_mt_' + i],
                delivaryPrice: formdata.dparr['dp_' + i],
            }
            ...
            orderDetailss.push(data)
        })

        console.log('orderDetails', orderDetailss)
        ...
        /*skuproperties.map((list,i)=>{
            ...
            var data={
                'skuMasterId':formdata.skuids['skuids_'+i],
                'qtyPcs':formdata.quantity_pcsarr['quantity_pcs_'+i],
                'qtyMT':formdata.quantity_mtarr['quantity_mt'+i],
                'delivaryPrice':formdata.RCP['dp_'+i],
            }
            skuproperties.map((skuproperties,indexinner) => {
                data['property_'+skuproperties.skuPropertiesMastersId]=formdata.proptype['proptype_'+skuproperties.skuPropertiesMastersId]
            })
            ...
            console.log('new form datat',data)
        })*/
        /* orderDetails= [
            {
                "skuMasterId": "1",
        ...
        var newformdata = {
            ...
            vehicalNumber: formdata.vehical_number,
            doc: formdata.doc_no,
            skuId: formdata.SKU,
            ...
            date: newdate,
            delivaryOrderDetails: orderDetailss,
            createdBy:LoginId,
        }
        ...
        response = await postDataFromApi(
            'dashboard/delivaryOrder/submitDelivaryOrder',
            newformdata
        )
                ...
                if (orderid) {
                    ...
                    if (orderDetails) {
                        orderDetails.map((orderDetails, index) => {
                            quantity_pcsarr[
                                'quantity_pcs_' + orderDetails.skuId
                            ] = orderDetails.quantity_pcs
                            quantity_mtarr[
                                'quantity_mt_' + orderDetails.skuId
                            ] = orderDetails.quantity_mt
                            rcparr['rcp_' + orderDetails.skuId] =
                                orderDetails.rcp
                        ...
                        propertyLabels.map((propertyLabels, index) => {
                            proptypearr[
                                'proptype_' +
                                    propertyLabels.skuPropertiesMastersId
        ...
        return (
            <Container>
                <div className="breadcrumb leftalign_breadcrumb">
                    <Breadcrumb routeSegments={[{ name: 'Logistic Order' }]} />
                </div>
                <Button
                            ...
                            <div className="datediv">
                                ...
                                >
                                    <DatePicker
                                        ...
                                        value={formdata.expectedDeleveryDate ? formdata.expectedDeleveryDate :null}
                           ...
                           <TextField
                            ...
                            <LocalizationProvider dateAdapter={AdapterDateFns}>
                                <DatePicker
                                    ...
                                    inputFormat="dd/MM/yyyy"
                                    value={formdata.transport_date ? formdata.transport_date :null}
                                    onChange={(e,name) => handleDateChange(e,'transport_date')}
                                    renderInput={(props) => (
                                        <TextField
                                            {...props}
                                            // variant="Outlined"
