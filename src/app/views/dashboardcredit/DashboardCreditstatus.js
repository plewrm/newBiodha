...
const AppTable = () => {
    ...
    const getBM = async (custumer_id="") => {
        var query = ''
        const response = await postDataFromApi('order/getBM/'+custumer_id, query)
        if (response && response.data.code && response.data.data != null) {
    ...
    const handledeleteConfirm = async () => {
        ...
        if (delete_id) {
            ...
            response = await postDataFromApi(
                'dashboard/allDashboard/orderCreaditTeamApprovalOrReject/' +
                    orderid,
                newformdata
            )
