import React, { useState } from 'react'
import { Box, styled } from '@mui/system'
import { useNavigate } from 'react-router-dom'
import { Span } from 'app/components/Typography'
import { Card, Grid, Button } from '@mui/material'
import { TextValidator, ValidatorForm } from 'react-material-ui-form-validator'
import {postDataFromApi,getDataFromApi,putDataFromApi} from '../../services/CommonService';


const TextField = styled(TextValidator)(() => ({
    width: '100%',
    marginBottom: '16px',
}))


const FlexBox = styled(Box)(() => ({
    display: 'flex',
    alignItems: 'center',
}))

const JustifyBox = styled(FlexBox)(() => ({
    justifyContent: 'center',
}))

const ContentBox = styled(Box)(() => ({
    height: '100%',
    padding: '32px',
    position: 'relative',
    background: 'rgba(0, 0, 0, 0.01)',
}))

const IMG = styled('img')(() => ({
    width: '100%',
}))

const ForgotPasswordRoot = styled(JustifyBox)(() => ({
    background: '#1A2038',
    minHeight: '100vh !important',
    '& .card': {
        maxWidth: 800,
        borderRadius: 12,
        margin: '1rem',
    },
}))

const ForgotPassword = () => {
    const navigate = useNavigate()
    const [state, setState] = useState({})
    const [isOTPsend, setisOTPsend] = useState(false)

    const handleChange = ({ target: { name, value } }) => {
        setState({
            ...state,
            [name]: value,
        })
    }

    const handleFormSubmit = async (event) => {
        
        if(isOTPsend){
           console.log(state)
           var query = "emailId="+email+"&otp="+otp+"&password="+password;
           const response = await postDataFromApi('login/validateOTP', query);
           console.log('otp',response)
        }else{
          var query = "emailId="+email;
          const response = await postDataFromApi('login/forgotPassword', query);
          console.log(response)
              response.data.code=1
              if(response.data.code){
                 setisOTPsend(true)
              }else{
                 setisOTPsend(false)
              }
        }
    }

    let { email, phoneno,otp,password} = state

    return (
        <ForgotPasswordRoot>
            <Card className="card">
                <Grid container>
                    <Grid item lg={5} md={5} sm={5} xs={12}>
                        <JustifyBox p={4} height="100%">
                            <IMG
                                src="/assets/images/illustrations/dreamer.svg"
                                alt=""
                            />
                        </JustifyBox>
                    </Grid>
                    <Grid item lg={7} md={7} sm={7} xs={12}>
                        <ContentBox>
                            <ValidatorForm onSubmit={handleFormSubmit}>
                                <TextField
                                    size="small"
                                    className="required"
                                    sx={{ mb: 3, width: '100%' }}
                                    variant="outlined"
                                    label="Email"
                                    onChange={handleChange}
                                    type="email"
                                    name="email"
                                    value={email || ''}
                                    validators={['required', 'isEmail']}
                                    errorMessages={[
                                        'this field is required',
                                        'email is not valid',
                                    ]}
                                />
                                {isOTPsend ? 
                                <TextField
                                    className="required"
                                    sx={{ mb: 3, width: '100%' }}
                                    variant="outlined"
                                    label="OTP"
                                    onChange={handleChange}
                                    type="text"
                                    name="otp"
                                    value={otp || ''}
                                    size="small"
                                    validators={['required']}
                                    errorMessages={['this field is required']}
                                />
                                :'' }
                                {isOTPsend ? 
                                <TextField
                                    className="required"
                                    sx={{ mb: '12px', width: '100%' }}
                                    label="New Password"
                                    variant="outlined"
                                    size="small"
                                    onChange={handleChange}
                                    name="password"
                                    type="password"
                                    value={password || ''}
                                    validators={['required','matchRegexp:^.{5,255}$']}
                                    errorMessages={['this field is required','Minimum 5 characters required']}
                                /> 
                                :'' }
                                <FlexBox>
                                    <Button
                                        variant="contained"
                                        color="primary"
                                        type="submit"
                                    >
                                    {isOTPsend ? 'Reset Password' :'Send OTP'}
                                    </Button>
                                    <Span sx={{ mr: 1, ml: '16px' }}>or</Span>
                                    <Button
                                        sx={{ textTransform: 'capitalize' }}
                                        onClick={() => navigate("/session/signin")}
                                    >
                                        Sign in
                                    </Button>
                                </FlexBox>
                            </ValidatorForm>
                        </ContentBox>
                    </Grid>
                </Grid>
            </Card>
        </ForgotPasswordRoot>
    )
}

export default ForgotPassword
